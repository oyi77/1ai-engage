...
async def list_contacts(
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(f"SELECT COUNT(*) FROM contacts WHERE {where_clause}", params)
        total = cursor.fetchone()[0]

    contacts = [
        Contact(
            id=row["id"], wa_number_id=row["wa_number_id"], name=row["name"],
            phone=row["phone"], email=row["email"], company=row["company"],
            notes=row["notes"], tags=row["tags"], source=row["source"],
        ) for row in rows
    ]
...
async def create_contact(contact: ContactCreate) -> ContactResponse:
    ...
    with get_db_connection(row_factory=False) as conn:
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (contact.wa_number_id, contact.name, contact.phone, contact.email,
              contact.company, contact.notes, contact.tags, contact.source))
        contact_id = cursor.lastrowid
        conn.commit()

    return ContactResponse(
        contact=Contact(
            id=contact_id, wa_number_id=contact.wa_number_id, name=contact.name,
            phone=contact.phone, email=contact.email, company=contact.company,
            notes=contact.notes, tags=contact.tags, source=contact.source,
        )
    )
...
async def update_contact(contact_id: int, update: ContactUpdate) -> ContactResponse:
    ...
    if update.wa_number_id is not None:
        update_fields.append("wa_number_id = ?"); params.append(update.wa_number_id)
    if update.name is not None:
        update_fields.append("name = ?"); params.append(update.name)
    if update.phone is not None:
        update_fields.append("phone = ?"); params.append(update.phone)
    if update.email is not None:
        update_fields.append("email = ?"); params.append(update.email)
    if update.company is not None:
        update_fields.append("company = ?"); params.append(update.company)
    if update.notes is not None:
        update_fields.append("notes = ?"); params.append(update.notes)
    if update.tags is not None:
        update_fields.append("tags = ?"); params.append(update.tags)
    if update.source is not None:
        update_fields.append("source = ?"); params.append(update.source)
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(f"UPDATE contacts SET {', '.join(update_fields)} WHERE id = ?", params)
...
def _ensure_contact_profile(conn, contact_id: int, wa_number_id: str):
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM contact_profiles WHERE contact_id = ?", (contact_id,))
    if cursor.fetchone():
        return
    cursor.execute(
        "INSERT INTO contact_profiles (contact_id, wa_number_id, profile_photo_url, status, is_business) VALUES (?, ?, NULL, NULL, 0)",
...
async def get_contact_profile(contact_id: int):
    ...
    contact = Contact(
        id=contact_row["id"], wa_number_id=contact_row["wa_number_id"], name=contact_row["name"],
        phone=contact_row["phone"], email=contact_row["email"], company=contact_row["company"],
        notes=contact_row["notes"], tags=contact_row["tags"], source=contact_row["source"],
    )
    profile = None
    if profile_row:
        profile = ContactProfile(
            id=profile_row["id"], contact_id=profile_row["contact_id"],
            wa_number_id=profile_row["wa_number_id"], profile_photo_url=profile_row["profile_photo_url"],
            status=profile_row["status"], is_business=bool(profile_row["is_business"]),
            business_name=profile_row["business_name"], business_description=profile_row["business_description"],
            address=profile_row["address"], website=profile_row["website"],
            birthday=profile_row["birthday"], custom_fields=profile_row["custom_fields"],
...
async def update_contact_profile(contact_id: int, update: ContactProfileUpdate):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, wa_number_id FROM contacts WHERE id = ?", (contact_id,))
        contact_row = cursor.fetchone()
        if not contact_row:
            raise HTTPException(status_code=404, detail="Contact not found")
        ...
        if update.profile_photo_url is not None:
            update_fields.append("profile_photo_url = ?"); params.append(update.profile_photo_url)
        if update.status is not None:
            update_fields.append("status = ?"); params.append(update.status)
        if update.is_business is not None:
            update_fields.append("is_business = ?"); params.append(1 if update.is_business else 0)
        if update.business_name is not None:
            update_fields.append("business_name = ?"); params.append(update.business_name)
        if update.business_description is not None:
            update_fields.append("business_description = ?"); params.append(update.business_description)
        if update.address is not None:
            update_fields.append("address = ?"); params.append(update.address)
        if update.website is not None:
            update_fields.append("website = ?"); params.append(update.website)
        if update.birthday is not None:
            update_fields.append("birthday = ?"); params.append(update.birthday)
        if update.custom_fields is not None:
            update_fields.append("custom_fields = ?"); params.append(update.custom_fields)
        ...
        cursor.execute(f"UPDATE contact_profiles SET {', '.join(update_fields)} WHERE contact_id = ?", params)
        conn.commit()
        cursor.execute("""
            SELECT id, contact_id, wa_number_id, profile_photo_url, status, is_business,
                   business_name, business_description, address, website, birthday, custom_fields,
