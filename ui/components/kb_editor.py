...
def render_kb_editor():
    ...
    try:
        ...
        wa_numbers = (
            [s.get("session_name") for s in sessions if s.get("session_name")]
            if sessions
            else ["default"]
        )
    ...
    st.session_state.kb_wa_number = st.selectbox(
        "WA Number (Session)",
        options=wa_numbers,
        index=wa_numbers.index(st.session_state.kb_wa_number)
        if st.session_state.kb_wa_number in wa_numbers
        else 0,
    ...
    search_query = st.text_input(
        "Test KB Search (real-time)", placeholder="Type a question or keywords..."
    )
    if search_query:
        try:
            ...
            if results:
                for r in results:
                    ...
                    with st.expander(
                        f"[{r['category'].upper()}] {r['question']} {score_display}"
                    ):
                        st.markdown(f"**Answer:** {r['answer']}")
                        if r.get("content"):
                            st.markdown(f"**Content:** {r['content']}")
                        st.caption(
                            f"Tags: {r.get('tags', '')} | Priority: {r.get('priority', 0)}"
                        )
            else:
            ...
            if selected_entry:
                ...
                with st.form(f"edit_form_{selected_id}"):
                    edit_cat = st.selectbox(
                        ...
                        index=["faq", "doc", "snippet"].index(
                            selected_entry.get("category", "faq")
                        )
                        if selected_entry.get("category") in ["faq", "doc", "snippet"]
                        else 0,
                    )
                    edit_q = st.text_input(
                        "Question / Title", value=selected_entry.get("question", "")
                    )
                    edit_a = st.text_area(
                        "Answer / Short Summary", value=selected_entry.get("answer", "")
                    )
                    edit_c = st.text_area(
                    ...
                    edit_t = col1.text_input(
                        "Tags (comma separated)", value=selected_entry.get("tags", "")
                    )
                    edit_p = col2.number_input(
                        "Priority", value=int(selected_entry.get("priority", 0)), step=1
                    )
