...
# ---------------------------------------------------------------------------
VOICE_STT_MODEL_SIZE = os.getenv(
    "VOICE_STT_MODEL_SIZE", "medium"
)  # tiny, base, small, medium, large-v3
VOICE_STT_DEVICE = os.getenv(
    "VOICE_STT_DEVICE", "cuda"
)  # cuda or cpu (fallback to cpu if gpu unavailable)
VOICE_STT_LANGUAGE = os.getenv("VOICE_STT_LANGUAGE", "id")  # Indonesian

# ---------------------------------------------------------------------------
# TTS (Text-to-Speech) Configuration
# ---------------------------------------------------------------------------
VOICE_TTS_ENGINE = os.getenv(
    "VOICE_TTS_ENGINE", "chatterbox"
)  # chatterbox or edge-tts (fallback)
VOICE_TTS_LANGUAGE_ID = os.getenv(
    "VOICE_TTS_LANGUAGE_ID", "ms"
)  # Malay (closest to Indonesian)
VOICE_TTS_EXAGGERATION = float(
    os.getenv("VOICE_TTS_EXAGGERATION", "0.5")
)  # Emotion control 0.0-1.0
VOICE_TTS_CFG_WEIGHT = float(
    os.getenv("VOICE_TTS_CFG_WEIGHT", "0.5")
)  # Pacing control 0.0-1.0
VOICE_TTS_DEVICE = os.getenv(
    "VOICE_TTS_DEVICE", "cuda"
)  # cuda or cpu (fallback for OOM)
...
# ---------------------------------------------------------------------------
VOICE_MAX_AUDIO_DURATION = int(
    os.getenv("VOICE_MAX_AUDIO_DURATION", "60")
)  # Max 60 seconds
VOICE_MAX_RESPONSE_LENGTH = int(
    os.getenv("VOICE_MAX_RESPONSE_LENGTH", "500")
)  # Max chars for TTS
VOICE_TIMEOUT_SECONDS = int(
    os.getenv("VOICE_TIMEOUT_SECONDS", "30")
)  # Max processing time before fallback
...
VOICE_SAMPLE_RATE = int(
    os.getenv("VOICE_SAMPLE_RATE", "24000")
...
def get_voice_config(session_name: str) -> dict:
    """Get voice configuration for a specific WAHA session from database."""
    try:
