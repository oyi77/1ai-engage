...
from utils import parse_display_name, safe_filename

_HEADERS = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/120"}
RESEARCH_DIR = "1ai-engage/data/research"

_PAIN_SIGNALS = {
    "no_chatbot":       lambda text: not any(w in text.lower() for w in ["chat", "tawk", "crisp", "intercom", "whatsapp"]),
    "no_blog":          lambda text: not any(w in text.lower() for w in ["blog", "artikel", "news", "berita"]),
    "no_testimonial":   lambda text: not any(w in text.lower() for w in ["testimoni", "review", "client", "klien", "case study"]),
    "no_cta":           lambda text: not any(w in text.lower() for w in ["contact us", "hubungi", "get started", "free consult", "konsultasi"]),
    "no_portfolio":     lambda text: not any(w in text.lower() for w in ["portfolio", "proyek", "project", "our work", "hasil"]),
}
...
def _extract_services(text: str) -> list[str]:
    ...
    keywords = [
        "SEO", "SEM", "Google Ads", "Meta Ads", "social media", "content marketing",
        "web development", "web design", "branding", "video production", "copywriting",
        "email marketing", "influencer", "KOL", "digital PR", "e-commerce",
        "mobile app", "UI/UX", "automation", "AI", "data analytics",
    ]
...
def research_prospect(website: str) -> dict:
    ...
    pages = [base, base.rstrip("/") + "/about", base.rstrip("/") + "/services",
             base.rstrip("/") + "/tentang-kami", base.rstrip("/") + "/layanan"]
    ...
    pain_points = [label for label, check in _PAIN_SIGNALS.items() if check(combined_text)]
    tech_stack = [name for name, signal in _TECH_SIGNALS.items() if signal.lower() in combined_html.lower()]
...
def process_research() -> None:
    ...
    for index, row in df.iterrows():
        ...
        df.at[index, "research"] = " | ".join(summary_parts) if summary_parts else "no_data"
        researched += 1
        time.sleep(0.5)
