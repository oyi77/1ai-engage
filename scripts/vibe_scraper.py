...
def _call_vibe(industry: str, location: str, n: int = LEADS_PER_QUERY) -> list[dict]:
    ...
    try:
        ...
        if not output:
            print(f"Vibe returned empty output. stderr: {result.stderr[:300]}", file=sys.stderr)
            return []

        # Extract JSON array from output (may be wrapped in ```json ... ```)
        json_match = re.search(r'\[.*\]', output, re.DOTALL)
        if not json_match:
            print(f"Could not find JSON array in Vibe output:\n{output[:500]}", file=sys.stderr)
            return []
...
def vibe_scrape(industry: str, location: str = "Jakarta, Indonesia",
                n: int = LEADS_PER_QUERY) -> None:
    ...
    if not df.empty:
        for val in df.get("websiteUri", pd.Series()).dropna():
            s = str(val).strip().lower().rstrip("/").replace("https://", "").replace("http://", "").replace("www.", "")
    ...
    for lead in raw_leads:
        ...
        site_key = website.lower().rstrip("/").replace("https://", "").replace("http://", "").replace("www.", "")
        ...
        row = {
            ...
            "phone":                    phone,
            "websiteUri":               website if website.startswith("http") else (f"https://{website}" if website else ""),
            "primaryType":              "service",
            "type":                     "Layanan",
