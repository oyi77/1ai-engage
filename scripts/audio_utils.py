...
def download_media(url: str, timeout: int = 30) -> bytes:
    ...
    if not url.startswith("http"):
        raise Exception(f"Invalid media URL: {url}")
    ...
    if not filename:
        raise Exception(f"Cannot extract filename from URL: {url}")
    ...
    if len(result.stdout) < 100:
        raise Exception(f"Downloaded file too small: {len(result.stdout)} bytes")
